...
if we use git branch -a to see all hidden files .as the story says the file we are looking for has been destroyed and erased .the lohg says that the newest one says that the vaults
 were removed and erased.so we should run the git checkout "d4e7bf5"
we have traveled back into time its like exactly how the vaults were before the destruction happened, there are 5 normal vaults and one secure vault which is hidden as it is
 starting with a dot.
...
which we used in the first level we get to see poneglyph.py which is also green in color .and as same as the first level the permissions given to this file are alo different
from others.
